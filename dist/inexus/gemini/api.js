...
exports.INexusGeminiApi = void 0;
...
const typewriting_1 = require("../typewriting");
class INexusGeminiApi {
    /**
     * NexusOpenaiApi
     * @param opts
     */
     ...
     * @param onAssistantId
     */
    async sendMessageBody(contents, config, options = {}, onAssistantId) {
        const messageId = options.message_id ? options.message_id : (0, uuid_1.v4)();
        ...
        if (options.request?.top_k)
            body.generationConfig.topK = options.request?.top_k;
        ...
        if (this._debug)
            console.log(`sendGeminiMessage (${assistantMessage.id.substring(0, 10)} ${config.key.substring(0, 10)} ${buildMessages.numTokens} tokens)`, JSON.stringify(body, null, "  "));
        let response;
        if (options.stream) {
            const typewriting = new typewriting_1.Typewriting((delta, text) => {
                options.onProgress?.({
                    ...assistantMessage,
                    delta: delta,
                    text: text,
            ...
            response = await this.sendMessageStreamRequest({
                ...
                onMessage: (message) => {
                    ...
                    if (delta?.content)
                        typewriting.append(delta?.content);
        ...
        for (const tool of response.tools) {
            const callingMessageId = (0, uuid_1.v4)();
     ...
     * @param options
     */
    async sendMessageStreamRequest(options) {
        return new Promise((resolve, reject) => {
            let tools = [];
            let response = null;
            (0, fetch_1.fetchSSE)(options.url, {
                method: 'POST',
                        ...
                        if (response.candidates?.length > 0) {
                            response.choices = response.candidates?.map((v) => {
                                ...
                                if (calls && calls?.length > 0)
                                    tools = tools.concat(calls);
                                return {
                                    index: v.index,
                                    delta: {
                                        role: v?.content?.role === 'model' ? 'assistant' : v?.content?.role,
                                        content: v?.content?.parts?.map((p) => p.text)?.join('\n'),
                                        tool_calls: calls
                                    },
     ...
     * @param options
     */
    async sendMessageRequest(options) {
        ...
        if (!response.ok) {
            ...
            const error = new types_1.INexusError(`Gemini error ${response.status || response.statusText}: ${reason}`);
     ...
     * @protected
     */
    async buildMessages(message, options) {
        ...
        if (maxInputToken - messageToken < 1)
            throw new Error(`发送的消息过长，请尝试减少消息内容,systemTokens:${systemToken},toolTokens:${toolsToken},textTokens:${messageToken}`);
        const messages = [];
        if (message.id)
        ...
        while (true) {
            if (!parentId)
                break;
            if (config.max_count > 0 && messages.filter(v => v.role === 'user').length > config.max_count)
                break;
            const parentMessage = await this.getMessage(parentId);
            if (!parentMessage)
                break;
            if (config.max_time > 0 && parentMessage.created_time && parentMessage.created_time + config.max_time < Date.now())
                break;
        ...
        for (const message of messages) {
            ...
            const requestMessage = { role: message.role, parts: [] };
            if (requestMessage.role === 'assistant')
                requestMessage.role = 'model';
            if (requestMessage.role === 'tool')
                requestMessage.role = 'function';
                ...
                requestMessage.parts = requestMessage.parts
                    ...
                    return {
                        functionCall: {
                            ...
                            args: (0, utils_1.toJson)(v.function.arguments)
                        }
                    };
                }));
            }
     ...
     * @param contents
     */
    getMessageContentsToken(contents) {
        if (!contents)
            return 0;
        let tokens = 0;
        for (const content of contents) {
            switch (content.type) {
                case "image":
                    if (content.image) {
                        tokens += this.getImageToken(content.image.width, content.image.height, content.image.mode);
                    }
                    break;
                case "text":
                default:
     ...
     * @param detail
     */
    getImageToken(width, height, detail) {
        return 259;
    }
    /**
     * 计算token数量
    ...
    getTextToken(...args) {
        ...
        return Math.ceil((0, gpt_3_encoder_1.encode)(text).length * 0.8);
    }
    /**
     ...
     * @private
     */
    convertRequestTool(tool) {
        const convert = (obj) => {
            if (!obj)
                return obj;
            for (const k in obj) {
                if (obj[k].enum)
                    obj[k].enum = obj[k].enum.map(v => String(v));
                if (obj[k].enum && obj[k].type)
                    obj[k].type = "string";
                if (obj[k].default)
                    delete obj[k].default;
                if (obj[k].examples)
                    delete obj[k].examples;
...
exports.INexusGeminiApi = INexusGeminiApi;
