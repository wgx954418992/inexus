...
exports.INexusOpenaiApi = void 0;
const types_1 = require("../types");
const gpt_3_encoder_1 = require("gpt-3-encoder");
const uuid_1 = require("uuid");
const fetch_1 = require("../fetch");
const utils_1 = require("../utils");
class INexusOpenaiApi {
    /**
     * NexusOpenaiApi
     * @param opts
     */
    ...
    async sendMessageBody(contents, config, options = {}, onAssistantId) {
        ...
        if (options.request?.top_k)
            body.top_k = options.request?.top_k;
        if (options.request?.tool_choice)
            body.tool_choice = options.request?.tool_choice;
        if (options.request?.tools)
            body.tools = options.request?.tools;
        if (config.options?.org)
            headers['OpenAI-Organization'] = config.options?.org;
        body.messages = buildMessages.messages;
        if (this._debug)
            console.log(`sendOpenaiMessage (${assistantMessage.id.substring(0, 10)} ${config.key.substring(0, 10)} ${buildMessages.numTokens} tokens)`, JSON.stringify(body, null, "  "));
        let response;
        if (options.stream) {
                        ...
                        response.choices?.forEach(choice => {
                            ...
                            choice.delta?.tool_calls?.forEach(call => {
                                if (!call?.function)
                                    return;
                                if (!tools[call.index])
                                    tools[call.index] = call;
                                tools[call.index].function = {
                                    ...tools[call.index]?.function,
                                    arguments: (tools[call.index].function?.arguments || "") + (call.function?.arguments || "")
                                };
                            });
                        });
    ...
    async sendMessageRequest(options) {
        ...
        if (!response.ok) {
            ...
            const error = new types_1.INexusError(`OpenAI error ${response.status || response.statusText}: ${reason}`);
    ...
    async buildMessages(message, options) {
        ...
        if (maxInputToken - messageToken < 1)
            throw new Error(`发送的消息过长，请尝试减少消息内容，或采用Gemini模型,systemTokens:${systemToken},toolTokens:${toolsToken},textTokens:${messageToken}`);
        const messages = [];
        if (message.id)
        ...
        if (options.systemMessage)
            requestMessages.push({ role: 'system', content: options.systemMessage });
        for (const message of messages) {
            ...
            const requestMessage = { role: message.role };
    ...
    getTextToken(...args) {
        ...
        return Math.ceil((0, gpt_3_encoder_1.encode)(text).length);
    }
}
exports.INexusOpenaiApi = INexusOpenaiApi;
